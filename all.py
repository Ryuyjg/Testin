        ...
        try:
            ...
            if password_info.has_password:
                ...
                try:
                    ...
                    # Change password using update_password method
                    try:
                        # First we need to get the password object again
                        password_info = await client(functions.account.GetPasswordRequest())
                        ...
                        await client(functions.account.UpdatePasswordSettingsRequest(
                            password=types.InputCheckPasswordSRP(
                                ...
                                M1=await client._compute_password_hash(
                                    password_info.current_algo,
                                    current_password
                                )
                            ),
...
async def option10_change_2fa():
    ...
    for idx, (session_name, session_file) in enumerate(selected_sessions, 1):
        ...
        if success:
            ...
            result_type = "ENABLED" if "ENABLED" in str(success) else "CHANGED"
            if result_type == "ENABLED":
                enabled_count += 1
            else:
                changed_count += 1
        else:
...
async def join_groups(session_path, session_name, group_usernames):
    """Join groups from a list with delays"""
    client = None
    try:
        api_creds = get_rotated_api()
        client = TelegramClient(session_path, api_creds['api_id'], api_creds['api_hash'])
        ...
        joined = 0
        total = len(group_usernames)
        
        for i, group_username in enumerate(group_usernames, 1):
            try:
                ...
                if i < total:
                    delay = random.uniform(MIN_DELAY_BETWEEN_GROUPS, MAX_DELAY_BETWEEN_GROUPS)
...
async def option11_join_groups():
    """Join groups from groups.txt file with delays between sessions"""
    display_banner()
    print(Fore.CYAN + "\n" + "═" * 60)
    print(Fore.GREEN + "👥 JOIN GROUPS FROM FILE")
    print(Fore.CYAN + "═" * 60)
    ...
    print(Fore.GREEN + f"✅ Found {len(group_usernames)} groups in {groups_file}")
    ...
    confirm = input(Fore.YELLOW + f"\n⚠️  Join {len(group_usernames)} groups with {len(selected_sessions)} sessions? (y/n): ").strip().lower()
    if confirm != 'y':
        print(Fore.YELLOW + "⏹️ Cancelled")
        return
    
    print(Fore.GREEN + f"\n⚡ JOINING GROUPS WITH {len(selected_sessions)} SESSIONS...\n")
    print(Fore.YELLOW + f"⏱️  Delays: {MIN_DELAY_BETWEEN_GROUPS}-{MAX_DELAY_BETWEEN_GROUPS}s between groups, {MIN_DELAY_BETWEEN_SESSIONS}-{MAX_DELAY_BETWEEN_SESSIONS}s between sessions\n")
    ...
    for idx, (session_name, session_file) in enumerate(selected_sessions, 1):
        ...
        joined, attempted = await join_groups(session_file, session_name, group_usernames)
        total_joined += joined
        total_attempted += attempted
        
        # DELAY BETWEEN SESSIONS
        if idx < len(selected_sessions):
            delay = random.uniform(MIN_DELAY_BETWEEN_SESSIONS, MAX_DELAY_BETWEEN_SESSIONS)
            print(Fore.BLUE + f"\n⏳ Waiting {delay:.1f}s before starting next session...")
            await asyncio.sleep(delay)
    
    print(Fore.CYAN + "\n" + "═" * 60)
    print(Fore.YELLOW + f"📊 RESULTS: {total_joined}/{total_attempted} group joins successful")
    print(Fore.CYAN + "═" * 60)
...
def show_main_menu():
    ...
    print(Fore.WHITE + "   10. 🔐 POWERFUL 2FA Password Manager")  # Updated option 10
    print(Fore.WHITE + "   11. 👥 Join Groups (from groups.txt)")
...
async def main():
    ...
    while True:
        try:
            ...
            elif choice == "9":  # New Check Session Validity
                await option9_check_sessions()
                input(Fore.YELLOW + "\n   Press Enter to continue...")
            
            elif choice == "10":  # POWERFUL 2FA Password Manager
                await option10_change_2fa()
                input(Fore.YELLOW + "\n   Press Enter to continue...")
            
            elif choice == "11":  # New Join Groups
                await option11_join_groups()
                input(Fore.YELLOW + "\n   Press Enter to continue...")
            
            elif choice == "12":  # New Terminate Sessions
                await option12_terminate_sessions()
                input(Fore.YELLOW + "\n   Press Enter to continue...")
            
            elif choice == "13":  # Updated exit option
