...
def create_session_control_keyboard(session_name: str) -> InlineKeyboardMarkup:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔄 Restart", callback_data=f"restart_{session_name}"),
         InlineKeyboardButton("⏹️ Stop", callback_data=f"stop_{session_name}")],
        ...
        [InlineKeyboardButton("🖼️ Photo", callback_data=f"photo_{session_name}"),
         InlineKeyboardButton("📤 Forward", callback_data=f"forward_{session_name}")],
        [InlineKeyboardButton("✏️ Rename", callback_data=f"rename_{session_name}"),
         InlineKeyboardButton("🔍 Info", callback_data=f"info_{session_name}")],
        [InlineKeyboardButton("🔙 Back to Sessions", callback_data="list_sessions_0"),
         InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]
    ]
...
def create_settings_keyboard(session_name: str) -> InlineKeyboardMarkup:
    ...
    auto_reply_icon = "✅" if settings['auto_reply'] else "❌"
    forward_icon = "✅" if settings['forward_messages'] else "❌"
    active_icon = "✅" if settings['active'] else "❌"
    profile_icon = "✅" if settings['change_profile'] else "❌"
    
    keyboard = [
        [InlineKeyboardButton(f"{auto_reply_icon} Auto Reply", 
                            callback_data=f"toggle_autoreply_{session_name}"),
         InlineKeyboardButton(f"{forward_icon} Forwarding", 
                            callback_data=f"toggle_forward_{session_name}")],
        [InlineKeyboardButton(f"{active_icon} Active", 
                            callback_data=f"toggle_active_{session_name}"),
         InlineKeyboardButton(f"{profile_icon} Auto Profile", 
...
def create_bulk_actions_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("⏹️ Stop 1-10", callback_data="bulk_stop_1_10"),
         InlineKeyboardButton("⏹️ Stop 11-20", callback_data="bulk_stop_11_20")],
        [InlineKeyboardButton("✏️ Rename Batch", callback_data="bulk_rename"),
         InlineKeyboardButton("📝 Bio All", callback_data="bulk_bio")],
        [InlineKeyboardButton("🖼️ Photo All", callback_data="bulk_photo"),
         InlineKeyboardButton("⚙️ Settings All", callback_data="bulk_settings")],
...
def create_main_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔄 System Refresh", callback_data="system_refresh"),
         InlineKeyboardButton("🚪 Exit", callback_data="exit_app")]
    ]
...
async def sessions_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, (name, session) in enumerate(state.sessions.items(), 1):
        ...
        text += f"   ⚙️ Settings: {'✅' if session.settings['active'] else '❌'}\n\n"
...
"""
        ...
        await add_session_dialog(query)
...
async def add_session_dialog(query):
    """Handle adding new session via dialog"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif user_data.get('adding_session'):
        await handle_session_creation(update, context)

async def handle_session_creation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle multi-step session creation"""
    step = context.user_data.get('session_step')
    
    if step == 'name':
        context.user_data['new_session'] = {'name': update.message.text}
        context.user_data['session_step'] = 'api_id'
        await update.message.reply_text("Send API ID:")
    
    elif step == 'api_id':
        try:
            context.user_data['new_session']['api_id'] = int(update.message.text)
        ...
        context.user_data['new_session']['api_hash'] = update.message.text
        context.user_data['session_step'] = 'string_session'
        await update.message.reply_text("Send String Session:")
    
    elif step == 'string_session':
        session_data = context.user_data['new_session']
        ...
        # Clear temp data
        del context.user_data['adding_session']
        del context.user_data['session_step']
        del context.user_data['new_session']
        ...
        await update.message.reply_text(
            f"<b>✅ Session '{session_name}' added!</b>",
