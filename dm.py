...
# ==================== NEW OPTION: DM FORWARDER (COPY-PASTE) - FIXED ====================
async def get_latest_dm_message(client, session_name, target_username):
    try:
        ...
        try:
            ...
            print(Fore.RED + f"[{session_name}] ❌ Can't access user @{target_username}: {str(e)[:50]}")
            return None
        
        # Get last 1 message from that user (latest message only)
        messages = []
        try:
            async for message in client.iter_messages(user, limit=1):
        ...
        if not messages:
            print(Fore.YELLOW + f"[{session_name}] ⚠️ No text messages found from @{target_username}")
            return None
        ...
        user_name = getattr(user, 'username', target_username)
        if not user_name or user_name == 'Unknown':
            user_name = getattr(user, 'first_name', 'User')
        
        msg_preview = selected_message.text[:50] + "..." if len(selected_message.text) > 50 else selected_message.text
        print(Fore.CYAN + f"[{session_name}] 📩 Latest message from @{user_name}: {msg_preview}")
...
async def process_groups_copy_dm(client, session_name, target_username):
    # Get latest DM message from target user
    message = await get_latest_dm_message(client, session_name, target_username)
...
async def run_dm_forwarder_session(session_path, session_name, target_username):
    ...
    try:
        ...
        print(Fore.YELLOW + f"[{session_name}] 🔄 Copying latest message from @{target_username} to groups")
...
async def option7_dm_forwarder():
    ...
    target_username = input(Fore.YELLOW + "\n🎯 Target username (copy latest message from): ").strip()
    if not target_username:
    ...
    print(Fore.YELLOW + f"\n📝 Will copy latest message from: @{target_username}")
